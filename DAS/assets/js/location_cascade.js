...
class LocationCascade {
    ...
    async loadMunicipalities() {
        if (!this.provinceSelect || !this.districtSelect) return;
        const province = this.provinceSelect.value;
    ...
    populateSelect(selectElement, options) {
        ...
        options.forEach(option => {
            ...
            if (typeof option === 'object' && option !== null) {
                // Handle object response (e.g. {id: "P1", name: "Koshi"})
                opt.value = option.name || option.id;
    ...
    populateSelectNumbers(selectElement, numbers) {
        ...
        // Helper to convert English digits to Nepali digits
        ...
        numbers.forEach(num => {
            const opt = document.createElement('option');
            opt.value = num; // Keep English for value/logic
            opt.textContent = toNepaliDigits(num); // Nepali for display
            selectElement.appendChild(opt);
        });
